...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('turtle.start', () => {
        TurtlePanel.createOrShow(context.extensionPath);
    }));
    context.subscriptions.push(vscode.commands.registerCommand('shapeMap.start', () => {
        ShapeMapPanel.createOrShow(context.extensionPath);
    }));
    context.subscriptions.push(vscode.commands.registerCommand('validation.start', () => {
...
class TurtlePanel {
    ...
    static createOrShow(extensionPath) {
        ...
        // Otherwise, create a new panel.
        const panel = vscode.window.createWebviewPanel(TurtlePanel.viewType, 'Turtle Editor', column || vscode.ViewColumn.One, {
            // Enable javascript in the webview
            enableScripts: true,
            // And restrict the webview to only loading content from our extension's `media` directory.
            localResourceRoots: [vscode.Uri.file(path.join(extensionPath, 'media'))]
    ...
    _getHtmlForWebview(webview) {
        // Local path to main script run in the webview
        const scriptPathOnDisk = vscode.Uri.file(path.join(this._extensionPath, 'media', 'yate.bundled.min.js'));
        const cssPathOnDisk = vscode.Uri.file(path.join(this._extensionPath, 'media', 'yate.min.css'));
        // And the uri we use to load this script in the webview
        const scriptUri = webview.asWebviewUri(scriptPathOnDisk);
        const cssUri = webview.asWebviewUri(cssPathOnDisk);
        // Use a nonce to whitelist which scripts can be run
...
class ShapeMapPanel {
    ...
    static createOrShow(extensionPath) {
        ...
        // Otherwise, create a new panel.
        const panel = vscode.window.createWebviewPanel(ShapeMapPanel.viewType, 'ShapeMap Editor', column || vscode.ViewColumn.One, {
            // Enable javascript in the webview
            enableScripts: true,
            // And restrict the webview to only loading content from our extension's `media` directory.
            localResourceRoots: [vscode.Uri.file(path.join(extensionPath, 'media'))]
    ...
    _getHtmlForWebview(webview) {
        // Local path to main script run in the webview
        const scriptPathOnDisk = vscode.Uri.file(path.join(this._extensionPath, 'media', 'yasme.bundled.min.js'));
        const cssPathOnDisk = vscode.Uri.file(path.join(this._extensionPath, 'media', 'yasme.min.css'));
        // And the uri we use to load this script in the webview
        const scriptUri = webview.asWebviewUri(scriptPathOnDisk);
        const cssUri = webview.asWebviewUri(cssPathOnDisk);
        // Use a nonce to whitelist which scripts can be run
...
ShapeMapPanel.viewType = 'shapeMap';
class ValidationPanel {
    ...
    static createOrShow(extensionPath) {
        ...
        // Otherwise, create a new panel.
        const panel = vscode.window.createWebviewPanel(ValidationPanel.viewType, 'Validation', column || vscode.ViewColumn.One, {
            // Enable javascript in the webview
            enableScripts: true,
            // And restrict the webview to only loading content from our extension's `media` directory.
            localResourceRoots: [vscode.Uri.file(path.join(extensionPath, 'media'))]
    ...
    _getHtmlForWebview(webview) {
        const yasheScriptPath = vscode.Uri.file(path.join(this._extensionPath, 'media', 'yashe.bundled.min.js'));
        const yateScriptPath = vscode.Uri.file(path.join(this._extensionPath, 'media', 'yate.bundled.min.js'));
        const yasmeScriptPath = vscode.Uri.file(path.join(this._extensionPath, 'media', 'yasme.bundled.min.js'));
        const mainScriptPath = vscode.Uri.file(path.join(this._extensionPath, 'media', 'main.js'));
        const yasheCSSPath = vscode.Uri.file(path.join(this._extensionPath, 'media', 'yashe.min.css'));
        const yateCSSPath = vscode.Uri.file(path.join(this._extensionPath, 'media', 'yate.min.css'));
        const yasmeCSSPath = vscode.Uri.file(path.join(this._extensionPath, 'media', 'yasme.min.css'));
        const stylePath = vscode.Uri.file(path.join(this._extensionPath, 'media', 'style.css'));
...
ValidationPanel.viewType = 'validation';
function getNonce() {
    let text = '';
